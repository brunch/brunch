...
const usesShim = (npmConfig, shim) => npmConfig.usedShims.indexOf(shim) !== -1;
const usesProcess = npmConfig => usesShim(npmConfig, 'process');

const addShims = (npmConfig, shims) => npmConfig.usedShims = uniq(npmConfig.usedShims.concat(shims));
const getUsedShims = npmConfig => npmConfig.usedShims;

const getShimData = shim => shims[shim];

module.exports = {emptyShims, fileShims, getShimData, makeSpecialShimFname, isSimpleShim, isShim, actualShimName, findGlobals, getUsedShims, usesProcess, addShims, shouldIncludeProcess};
