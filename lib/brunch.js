(function() {
  ...
  exports.compilePackage = function() {
    return package.compile(function(err, source) {
      ...
      return fs.writeFile(path.join(exports.options.brunchPath, 'build/web/js/app.js'), source, function(err) {
        if (err) {
          console.log(colors.lred(err, true));
        }
  ...
  exports.spawnStylus = function() {
    ...
    executeStylus = spawn('stylus', ['--compress', '--out', path.join(exports.options.brunchPath, 'build/web/css'), path.join(exports.options.brunchPath, 'src/app/styles/main.styl')]);
