...
const startServer = (config, callback) => {
  ...
  if (config.server.path) {
    ...
    const serverConfig = helpers.extend(opts, serverOpts.config || {});
...
const initWatcher = (config, callback) => {
  ...
  each(watched, fsexists, (err, existing) => {
...
const getCompileFn = (config, joinConfig, fileList, optimizers, watcher, preCompile, callback) => {
  return (startTime, watcherReady) => {
     ...
     */
    const writeCb = (error, generatedFiles, disposed) => {
      if (error != null) {
  ...
  return helpers.loadConfig(options.config, configParams, (error, config) => {
    ...
    const launchWatcher = () => {
      ...
      return initWatcher(config, (error, watcher) => {
        ...
        const compile = getCompileFn(config, joinConfig, fileList, optimizers, watcher, callPreCompillers, callCompileCallbacks);
...
const bindWatcherEvents = (config, fileList, compilers, linters, watcher, reload, onChange) => {
  ...
  const ref = config.paths;
  const packageConfig = ref.packageConfig;
  const bowerConfig = ref.bowerConfig;
  const jsonConfigs = [packageConfig, bowerConfig];
