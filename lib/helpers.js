...
const loadComponents = (config, type, callback) => {
  return readComponents('.', type, (error, components, aliases) => {
    ...
    const order = components.sort((a, b) => {
...
const loadNpm = (config, cb) => {
  if (!config.npm.enabled) return cb({components: []});
...
exports.loadConfig = (configPath, options, callback) => {
  ...
  try {
    ...
    const prePath = sysPath.resolve(configPath);
    const fullPath = require.resolve(prePath);
    ...
    if (configPath === defaultConfigFilename && error.code === 'MODULE_NOT_FOUND') {
