...
const throwError = function(type, stringOrError) {
  var error, string;
...
const lint = function(data, path, linters, callback) {
  if (linters.length === 0) {
...
const getDependencies = function(data, path, compilerDeps, compiler, callback) {
  var name;
...
const mapCompilerChain = function(compiler) {
...
const compile = function(source, path, compilers, callback) {
  var first;
  first = function(next) {
    return next(null, {
      source: source,
...
const pipeline = function(path, linters, compilers, callback) {
  if (isNpm(path)) {
