...
const checkDeps = require('micro-check-dependencies').checkRoot;
...
const addPackageManagers = async config => {
  config._normalized.packageInfo.npm = await loadNpm(config);
  console.log(13);
  return config;
};
...
const checkMatchedDeps = async (config) => {
  const packageDir = config.paths.root;
  const pkgs = await checkDeps(packageDir, !config._normalized.isProduction);
  console.log(100, pkgs);
  if (!pkgs.length) return config;

  const pkgPath = pkg => sysPath.join(packageDir, 'node_modules', pkg);
  const isNotSymlink = pkg => helpers.isSymlink(pkgPath(pkg)).then(x => !x);
  const unmetPkgs = await helpers.asyncFilter(pkgs, isNotSymlink);

  if (!unmetPkgs.length) return config;
  logger.info(`Using outdated versions of ${unmetPkgs.join(', ')}, trying to update to match package.json versions`);
  return install({rootPath: packageDir, pkgType: 'package'}).then(() => config);
};
