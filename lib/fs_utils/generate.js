...
const {flatten, promiseReduce, formatOptimizerError} = require('../utils/helpers');
...
const runOptimizer = optimizer => genFile => {
  ...
  const unoptMap = genFile.map;
  const path = genFile.path;
  const sourceFiles = genFile.sourceFiles;
