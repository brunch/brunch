...
const compilationProgress = () => {
  ...
  return setInterval(() => {
    const msg = iters === 7 ? 'still compiling' : 'compiling';
    logger.info(msg + '.'.repeat(iters % 4));
    iters++;
  }, 4000);
};
...
const mergeHooks = (plugins, config) => {
  return Object.keys(plugins).reduce((mergedHooks, name) => {
    ...
    mergedHooks[name] = (...args) => {
      return allHooks.map(plugin => plugin[name](...args));
    };
 ...
 */
class BrunchWatcher {
  constructor(options, partConfig) {
    ...
    const {env, onCompile} = options;
    loadConfig(partConfig).then(cfg => {
      this.config = cfg;
      return Promise.all([
  ...
  restartBrunch(pkgType) {
    const restart = () => {
      const newWatcher = new BrunchWatcher(this._options, this._partConfig);
