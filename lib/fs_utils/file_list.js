...
class FileList extends EventEmitter {
  ...
  get assetErrors() {
    return this.assets.filter(asset => asset.error).map(formatError);
  }
  ...
  isIgnored(path, test) {
    ...
    switch (toString.call(test).slice(8, -1)) {
      case 'RegExp': return test.test(path);
  ...
  dispose() {
    ...
    ['reading', 'compiling', 'copying', 'compiled', 'files', 'staticFiles'].forEach(key => {
      this[key].clear();
      this[key] = null;
    });
    this.assets = [];
