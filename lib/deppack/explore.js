...
const exploreDeps = npmConfig => fileList => {
  ...
  return x => {
    ...
    if (!isJs(fileList, path) || path.indexOf('.json') !== -1) {
    ...
    const deps1 = isApp(path) ? allDeps.filter(x => !helpers.isRelative(x)) : isPackage(path) ? allDeps.filter(shouldIncludeDep(npmConfig.rootPackage, path)) : allDeps;
