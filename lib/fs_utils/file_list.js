...
class FileList extends EventEmitter {
  constructor(config, watcher) {
    ...
    this.initial = true;
    this.disposed = false;
    this._previouslyCompiling = 0
    this._iteartionsWithoutChange = 0
  ...
  checkHangingCompilations() {
    if (this._previouslyCompiling === this.compiling.size) {
      this._iteartionsWithoutChange++
      // Entering the warning block only once to avoid cluttering the logs
      if (this._iteartionsWithoutChange === 15) {
        this.compiling.forEach((file) => {
          logger.warn(`${file} compilation is not responding: its compilator might have failed silently`);
        })
      }
    } else {
      this._iteartionsWithoutChange = 0
      this._previouslyCompiling = this.compiling.size
    }
  }
