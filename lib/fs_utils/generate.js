...
const runOptimizer = (optimizer, params) => {
  ...
  return processJob(OptimizeJob, {optimizer, params}).then(optimized => {
    const obj = toString.call(optimized) === '[object Object]';
    const optimizedCode = obj ? optimized.data : optimized;
    const optimizedMap = obj ? optimized.map : null;
    const map = optimizedMap == null ?
      unoptMap : prepareSourceMap(optimizedMap, sourceFiles);
    return Promise.resolve({
      data: optimizedCode, code: optimizedCode,
      map, path, sourceFiles
    });
  });
};
