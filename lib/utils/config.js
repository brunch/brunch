...
const loadConfig = (env, partialConfig) => {
  const configPath = partialConfig.paths.config;

  return tryToLoad(configPath)
    .then(validateConfig)
    .then(checkNpmModules)
    .then(setConfigDefaults(configPath))
    .then(addDefaultServer)
    .then(applyOverrides(env))
    .then(config => helpers.deepAssign(config, partialConfig))
