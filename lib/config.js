...
const replaceConfigSlashes = exports.replaceConfigSlashes = config => {
  ...
  const slashifyJoinTo = joinTo => {
    switch (toString.call(joinTo)) {
      ...
      case '[object Object]':
        const newJoinTo = {};
        Object.keys(joinTo).forEach(joinToKey => {
          return newJoinTo[replaceSlashes(joinToKey)] = joinTo[joinToKey];
        });
        return newJoinTo;
    }
  };
...
const createJoinConfig = configFiles => {
  ...
  const joinConfig = types.map(type => normalizeJoinConfig(configFiles[type].joinTo))
  .map((subCfg, index) => {
    ...
    if (fileCfg.entryPoints) {
      ...
      Object.keys(fileCfg.entryPoints).forEach(target => {
        ...
        Object.keys(normalizedEntryCfg).forEach(path => {
          ...
          outPaths.push(path);
          filteredEntryCfg[path] = match;
        });
        entryPoints[type][target] = filteredEntryCfg;
      });
    }
  });
