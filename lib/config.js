...
const readComponents = pify(require('read-components'));
...
const normalizeConfig = config => {
  ...
  Object.assign(norm, {
    ...
    conventions,
    packageInfo: {},
    isProduction: process.env.NODE_ENV === 'production', // deppack
    watcher: {
...
const loadBower = config => {
  ...
  return readComponents(rootPath, 'bower').then((components = []) => {
    const order = flat(components.sort((a, b) => {
    ...
    if (config.bower.enabled) {
      switch (error.code) {
        case 'NO_BOWER_JSON':
          logger.error('You probably need to execute `bower install` here.', error);
          return install({rootPath, pkgType: 'bower'}).then(() => loadBower(config));
        case 'ENOENT':
          logger.error(error);
          break;
      }
...
const addPackageManagers = config => {
  if (isWorker) return Promise.resolve(config);
  ...
  return Promise.all([
    ...
    [norm.npm, norm.bower] = components;
    return config;
  });
};

const minimalConfig = `Here's a minimal brunch-config.js to get you started:
...
const checkProjectDependencies = config => {
  ...
  const scopeList = process.env.NODE_ENV === 'production' ?
    ['dependencies'] :
...
const raw = {};
const norm = {};
const init = partConfig => {
  const {env, paths} = partConfig;

  return tryToLoad(paths.config)
    .then(validateConfig)
    .then(checkNpmModules)
    .then(setConfigDefaults(paths.config))
    .then(addDefaultServer)
    .then(applyOverrides(env))
    .then(config => deepAssign(config, partConfig))
    .then(normalizeConfig)
    .then(checkProjectDependencies)
    .then(addPackageManagers)
    .then(deepFreeze(dontFreeze));
};
