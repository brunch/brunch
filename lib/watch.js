...
const plugins = require('./utils/plugins'); // init, isPluginFor
...
const mergeHooks = (plugins, config) => {
  return Object.keys(plugins).reduce((mergedHooks, name) => {
    const pluginsHooks = plugins[name];
    const configHook = config[name];
    if (typeof configHook === 'function') {
      pluginsHooks.push(configHook);
    }

    mergedHooks[name] = function() {
      // => has lexical `arguments`
      return pluginsHooks.map(plugin => {
...
class BrunchWatcher {
    ...
    application.loadConfig(persistent, options)
      ...
      .then(cfg => {
        ...
        return Promise.all([
          ...
          plugins.init(cfg, onCompile, options.dependencies),
        ]);
      })
      .then(res => {
  ...
  compile(startTime, watcherReady) {
    ...
    Promise.all(this.hooks.preCompilers()).then(() => {
