...
const checkProjectDependencies = config => {
  ...
  return checkDeps(packageDir, !config._normalized.isProduction).then(pkgs => {
    ...
    return helpers.asyncFilter(pkgs, isNotSymlink).then(unmetPkgs => {
      ...
      return installDeps(packageDir, {logger});
    });
  }).then(() => config);
};
...
function validateBrunchConfig(obj, basename) {
...
async function tryToLoad(root, configPath, pkg) {
  // read package.json
  const path = sysPath.join(root, 'package.json');
  const relativeConfigPath = sysPath.join(root, configPath + '.js');
  let brunchConfig;

  if (pkg.brunch) {
    // Case a): package.json with `brunch` field exists
    throw new Error('NOT_IMPLEMENTED');
  } else if (await exists(relativeConfigPath)) {
    (relativeConfigPath);
    // Case b): brunch-config.js exists
    const fullPath = require.resolve(sysPath.resolve(relativeConfigPath));
    ...
    brunchConfig = DEFAULT_CONFIG_CONTENTS;
    // Check if coffeescript configs from brunch 2 exist,
    // Emit a warning if they do.
    // Don't block main thread with it.
    exists(sysPath.join(root, 'brunch-config.coffee'))
      .then(e1 => e1 || exists(sysPath.join(root, 'config.coffee')))
      .then(anyExists => {
        if (anyExists) {
...
async function loadConfig(options) {
  ...
  const pkg = loadPackageJSON(rootPath);
  ...
  helpers.deepAssign(config, params);
