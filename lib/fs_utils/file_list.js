...
class FileList {
  ...
  resetTimer() {
    ...
    this.timer = setTimeout(() => {
      ...
      if (!Object.keys(compiling).length && !Object.keys(copying).length) {
        this.emit('ready');
        this.compiled = {};
      } else {
        this.resetTimer();
  ...
  compile(file) {
    ...
    if (this.compiling[path]) {
      ...
      return file.compile(error => {
        ...
        return this.emit('compiled', path);
      });
    }
  };
  ...
  _change(path, compiler, linters, isHelper) {
    ...
    if (this.is('assets', path)) {
      ...
      debug("Reading '" + path + "'");
      return fcache.updateCache(path, (error, source) => {
        if (error) {
          if (error != null) {
