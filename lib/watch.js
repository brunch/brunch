...
class BrunchWatcher {
  constructor(persistent, options, onCompile) {
    ...
    this._isPreCompileFinished = false;
    this._isPreCompileStarted = false;
    this._filesToCompile = [];
  ...
  // Execute preCompile hook, collect files to compile
  // Start compilation off all files after preCompile hook finished work
  preCompilation(type, path) {
    if (!this._isPreCompileStarted) {
      this._isPreCompileStarted = true;

      Promise.all(this.hooks.preCompile())
        .then(() => {
          this._isPreCompileFinished = true;

          for (var i = 0; i < this._filesToCompile.length; i++) {
            this.startCompilation(this._filesToCompile[i][0], this._filesToCompile[i][1]);
          }
        });
    }

    this._filesToCompile.push([type, path]);
  }

  // Set start time of last compilation to current time.
  // Returns number.
  startCompilation(type, path) {
    if (!this._isPreCompileFinished) {
      this.preCompilation(type, path);
      return;
    }
