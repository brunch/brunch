...
const getGlobals = (npmConfig) => {
  const globals = Object.assign({}, npmConfig.npm.globals || {});
...
const processFiles = npmConfig => (root, files, processor) => {
  const usesProcess = npmConfig.usedShims.usesProcess();
  const _aliases = Object.assign({}, npmConfig.npm.aliases || {}, modules.requiredAliases(npmConfig));

  const aliases = Object.keys(_aliases).map(target => moduleDefinitions.aliasDef(target, _aliases[target]));

  const used = npmConfig.usedShims.toArray();
