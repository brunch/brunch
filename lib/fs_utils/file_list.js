...
// File list: A list of SourceFiles that contains *all* files from Brunches app.
...
class FileList {
  ...
  hasFiles() {
    for (const value in this.compiling) return true;
    for (const value in this.copying) return true;
    return false;
  }
  ...
  _change(path, compiler, linters, isHelper) {
    ...
    if (this.is('assets', path)) {
      ...
      fcache.updateCache(path, (error, source) => {
        if (error) return console.log('Reading', error);
        if (!ignored && (compiler && compiler.length)) {
          const sourceFile = this.find(path) || this._add(path, compiler, linters, isHelper);
