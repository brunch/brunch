...
const normalizePath = (path) => path.split('/').join(sysPath.sep);
...
class FileList extends EventEmitter {
  ...
  close() {
    ...
    const dispose = file => file.dispose();
    this.assets.forEach(dispose);
    this.files.forEach(dispose);
    ['compiling', 'copying', 'compiled', 'files'].forEach(k => this[k].clear());
