...
const getFormattedError = (error, code, path) => {
  const norm = typeof error === 'string' ? new Error(error) : error;
  const message = norm.message.trim().replace(/^/gm, '   ').trim();
  let details = '';
  if ('line' in error) {
    details += ` at L${error.line}`;
    if (error.col) details += `-${error.col}`;
  }
...
const getCompilationProgress = timePassed => {
  ...
  const writeWithDots = () => {
    ...
    logger.info(msg + '.'.repeat(iterations % 4));
    iterations++;
    timeout = setTimeout(writeWithDots, animationLogInterval);
  };

  let timeout = setTimeout(writeWithDots, initRunIn);
