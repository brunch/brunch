...
/*
 * options    - Object: {configPath, optimize, server, port, onCompile}.
...
class BrunchWatcher {
  constructor(options) {
    ...
    const onCompile = options.onCompile;
  ...
  restartBrunch(pkgType) {
    const restart = () => {
      ...
      if (process.send && process.env.BRUNCH_FORKED_PROCESS === 'true') {
        ...
        const newWatcher = new BrunchWatcher(this._constructorOptions);
...
module.exports = options => new BrunchWatcher(options);
