...
const getNewHeader = (moduleName, source, filePath) => {
  ...
  const r = isMain(filePath) ?
    "function(path) { return require(path.replace('./', '" + p2 + "/')); }" :
    'require';
  ...
  if (filePath.indexOf('.json') === -1) {
    ...
    return `require.register("${moduleName}", function(exports, require, module) {
      var relative = __makeRequire(${r}, ${JSON.stringify(brMap)}, '${getModuleFullRootName(filePath)}');
      ${glob}(function(exports, require, module) {
        ${source}
      })(exports, relative, module);
    });\n${fbAlias}\n`;
  } else {
    return `require.register("${moduleName}", function(exports, require, module) {
...
const makeRequire = (
  `var __makeRequire = function(require, mappings, pref) {
      var none = {};
