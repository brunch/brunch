...
const browserMappings = filePath => {
  ...
  if (browser && typeof browser === 'object') {
    ...
    const obj = {};
    if (!getMainCached(filePath)) return {};
...
const unexpandedFilePath = filePath => {
  const brMap = browserMappings(filePath);
...
const _mainFile = (root, json) => {
  ...
  return helpers.isDir(fileOrDir).then(isDir => {
    ...
    return unexpandedFilePath(main);
  });
};
...
const getNewHeader = (moduleName, source, filePath, origPath) => {
  ...
  if (filePath.indexOf('.json') === -1) {
    ...
    const p = origPath.replace(getModuleRootPath(origPath), '').replace('.json', '').replace('.js', '').split(sysPath.sep).slice(0, -1);
