...
const actual = (path, source, linters, compilers) => {
...
exports.pipeline = (path, linters, compilers, fileList) => {
  if (deppack.isNpm(path)) {
    ...
    const _compileAndLint = (source) => {
      return actual(path, source, linters, compilers);
    };

    const scheduleCompile = (source) => {
      return mediator.wrkr.schedule('compile', { path, source });
    };
    ...
    }).then(mediator.wrkr ? scheduleCompile : _compileAndLint).then(deppack.exploreDeps(fileList));
  }
};
