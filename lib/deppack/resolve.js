...
const buildModMap = npmConfig => {
  ...
  return glob(`{${topPaths}}/**/*`).then(mods => {
    ...
    npmConfig.modMap = Object.assign({}, modMap0, shimModMap, shimFileMap);
  });
};
...
const resolve = (npmConfig, mod, opts, cb) => {
  Object.assign(opts, {packageFilter: (pkg => helpers.applyPackageOverrides(pkg, npmConfig)), modules: npmConfig.modMap, extensions: ['.js', '.json']});
