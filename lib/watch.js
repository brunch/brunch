...
const getReloadFn = (config, options, onCompile, watcher, server, plugins) => {
  return reInstall => {
    ...
    if (reInstall) {
      return helpers.install(config.paths.root, 'npm').then(reWatch, reWatch);
    } else {
...
const initialize = (options, configParams, onCompile) => {
  ...
  return helpers.loadConfig(options.config, configParams).then(config => {
  ...
  watcher
    ...
    .on('change', absPath => {
      ...
      if (isConfigFile || isPackageFile) {
        ...
        return helpers.install(config.paths.root, 'bower').then(reload, reload);
      } else {
        return changeHandler(path);
      }
    })
