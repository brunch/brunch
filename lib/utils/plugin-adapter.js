...
const promisifyMethod = (plugin, key) => {
  ...
  if (arity === 2) {
    if (key === 'lint' || key === 'getDependencies') {
      ...
      promisified = file => promisify(fn).call(plugin, file);
    }
  }
  ...
  if (arity === 3) {
    promisified = file => promisify(fn).call(plugin, file.data, file.path);
  }

  if (arity >= 4) throw new Error(`Invalid arity ${arity}. Report this error to Brunch tracker.`)

  // console.log('Wrapped', plugin.brunchPluginName, key, arity);
