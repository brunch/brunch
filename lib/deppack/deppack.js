...
class Deppack {
  constructor(config, json) {
    ...
    this.getPackageJson = filePath => {
      return helpers.packageJson(filePath, npmConfig.rootPackage);
    };
  }
  ...
  exploreDeps(sourceFile) {
    const npmConfig = this.npmConfig;
    return explore(npmConfig.depGraph, npmConfig.modMap, npmConfig.usedShims, this.getPackageJson, npmConfig.isProduction, this.fileReflection, this.resolve)(sourceFile);
  }

  processFiles(root, files, processor) {
    const npmConfig = this.npmConfig;
    const requiredAliases = modules.requiredAliases(npmConfig.rootMainCache, this.getPackageJson, npmConfig.usedShims);
    processFiles(npmConfig.npm.aliases || {}, npmConfig.globals || {}, npmConfig.usedShims, requiredAliases)(root, files, processor);
  }

  wrapInModule(filePath) {
    filePath = sysPath.resolve('.', filePath);
    const npmConfig = this.npmConfig;
    return readFile(filePath).then(src => modules.wrap(this.getPackageJson, npmConfig.rootMainCache, filePath, src));
  }

  needsProcessing(file) {
    return file.path.indexOf('node_modules') !== -1 &&
      /\.(json|js)$/.test(file.path) &&
  ...
  isNpm(path) {
    return path.indexOf('node_modules') >= 0 &&
      !brunchRe.test(path) && !/\.css$/.test(path) &&
      this.fileReflection.isPackage(path) ||
      path.indexOf(dir) === 0;
  }

  isNpmJSON(path) {
    return this.isNpm(path) && /\.json$/.test(path);
  }
