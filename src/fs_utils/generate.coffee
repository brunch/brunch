...
# New.
concat = (files, path, type, definition, aliases, appendRequire) ->
...
generate = (path, sourceFiles, config, optimizers, callback) ->
  ...
  {code, map} = concat sorted, path, type, config._normalized.modules.definition, config._normalized.packageInfo.component.aliases, config._normalized.modules.appendRequire[joinKey]
