...
const callPlugin = (plugin, method, file) => {
  ...
  const callb = (error, data) => {
    if (error) fns.reject(error);
    else fns.resolve(data);
  };
  ...
  return safePromise(isPromise(result) ? result : promise,
    warningLogInterval,
    `${plugin.brunchPluginName} is taking too long to ${method} @ ${file.path}`
  );
};
...
const formatError = file => {
  ...
  return `${error.pipelineCode} of ${file.path} failed. ${error.message}`;
};
...
const promisifyHook = plugin => {
  ...
  if (typeof hook !== 'function' || !hook.length) return;
