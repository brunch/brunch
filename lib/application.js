...
const loadNpm = (config) => {
  return new Promise((resolve, reject) => {
    try {
      mediator.npmIsEnabled = config.npm.enabled;
      ...
      return deppack.loadInit(config, json).then(resolve, reject);
    } catch (e) {
      return reject(e);
    }
  });
...
const tryToLoad = (configPath, fallbackHandler) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (error.code === 'MODULE_NOT_FOUND' && isConfigRequireError) {
      if (!fallbackHandler) {
        fallbackHandler = () => {
          logger.error(`The current directory does not seem to be a Brunch project. Create ${basename}.js or run brunch from the correct directory. ${error.toString().replace('Error: ', '')}`);
          process.exit(0);
        };
      }
