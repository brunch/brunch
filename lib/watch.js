...
const generateParams = function(persistent, options) {
  var env, params;
  env = options.env;
  params = {
    env: (env && env.split(',')) || []
...
const startServer = function(config, callback) {
...
const initWatcher = function(config, callback) {
...
const changeFileList = function(compilers, linters, fileList, path, isHelper) {
...
const generateCompilationLog = function(startTime, allAssets, generatedFiles, disposedFiles) {
...
const getCompileFn = function(config, joinConfig, fileList, optimizers, watcher, preCompile, callback) {
...
const getReloadFn = function(config, options, onCompile, watcher, server, plugins) {
  return reInstall => {
    var reWatch;
    reWatch = function() {
      var restart;
...
const getPlugins = function(packages, config) {
  var exts;
...
const loadPackages = function(rootPath, callback) {
...
const initialize = function(options, configParams, onCompile, callback) {
...
const isConfigFile = function(basename, configPath) {
  var files;
  files = Object.keys(require.extensions).map(_ => {
    return configPath + _;
  });
...
const bindWatcherEvents = function(config, fileList, compilers, linters, watcher, reload, onChange) {
  ...
  watcher.on('error', logger.error).on('add', absPath => {
    var path;
...
class BrunchWatcher {
  constructor(persistent, options, onCompile) {
    ...
    initialize(options, configParams, onCompile, function(error, result) {
      var compile, compilers, config, fileList, includes, linters, reload, watcher, watcherReady;
      if (error != null) {
        return logger.error(error);
      }
...
const watch = module.exports = function(persistent, path, options, callback) {
  if (callback == null) {
    callback = (function() {});
  }
