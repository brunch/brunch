...
const requiredAliases = (npmConfig) => {
  ...
  return Object.assign(aliases, shimAliases(npmConfig.usedShims));
};

const browserMappings = (filePath, mainFile, npmConfig) => {
  const pkg = packageJson(filePath, npmConfig.rootPackage);
...
const _getMainFile = (npmConfig, filePath) => {
  ...
  const json = packageJson(filePath, npmConfig.rootPackage);
