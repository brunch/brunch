...
const browserMappings = filePath => {
  const browser = packageJson(filePath).browser;
  return (browser && typeof browser === 'object') ? browser : {};
};
...
const getNewHeader = (moduleName, source, filePath) => {
  ...
  const _parsed = require('esprima').parse(source);
  const useStrict = _parsed.body && _parsed.body[0] && _parsed.body[0].type === 'ExpressionStatement' && _parsed.body[0].expression.value === 'use strict';
