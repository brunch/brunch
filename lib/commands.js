...
program.command('watch [path]')
  ...
  .option('-e, --env [setting]', 'specify a set of override settings to apply')
  .option('-p, --production', 'same as `--env production`')
  .option('-s, --server', 'run a simple http server for the public dir on localhost (set server/hostname to override)')
  .option('-a, --all-ips', 'if `server` was given, listen on all ipv4 addresses (same as server/hostname of 0.0.0.0)')
