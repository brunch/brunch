...
const exploreDeps = npmConfig => fileList => {
  if (!fList) {
    fList = fileList;
    if (npmConfig.npmIsEnabled) fileList.on('unlink', onUnlink);
  }
  return x => {
    if (!x) return;
    const path = x.path;
    ...
    const allDeps = isVendor(npmConfig)(path) ? [] : detective(source);
    const usesProcess = isVendor(npmConfig)(path) ? false : shims.shouldIncludeProcess(source);
    const deps1 = isApp(npmConfig)(path) ? allDeps.filter(x => !helpers.isRelative(x)) : isPackage(npmConfig)(path) ? allDeps.filter(shouldIncludeDep(path)) : allDeps;
