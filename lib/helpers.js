...
const applyOverrides = (config, options) => {
  ...
  environments.forEach(override => {
    ...
    overrideProps = ((ref = config.overrides) != null ? ref[override] : undefined) || {};
    ...
    for (k in ref1) {
      v = ref1[k];
      if ((ref2 = config.plugins) != null ? ref2[v] : undefined) {
...
const install = exports.install = (rootPath, command, callback) => {
  var prevDir;
  if (callback == null) {
    callback = (() => {});
  }
  ...
  return exec(command + " install", (error, stdout, stderr) => {
    var log;
...
const createJoinConfig = configFiles => {
  ...
  types.forEach(type => {
    ...
    return joinConfig[type].pluginHelpers = Array.isArray(pluginHelpers) ? pluginHelpers : pluginHelpers ? [pluginHelpers] : (() => {
...
const getModuleWrapper = (type, nameCleaner) => {
  return (fullPath, data, isVendor) => {
    ...
    if (isVendor) {
      ...
      if (type === 'commonjs') {
        return {
          prefix: "require.register(" + path + ", (exports, require, module) => {\n",
          suffix: "});\n\n"
        };
      } else if (type === 'amd') {
        return {
...
const bowerRe = /NO_BOWER_JSON/;
const loadComponents = (config, type, callback) => {
  return readComponents('.', type, (error, components, aliases) => {
    var order;
    if (error) {
      let errStr = error.toString();
      if (error.code === 'NO_BOWER_JSON') {
        logger.error('You probably need to execute `bower install` here. ' +error);
      } else if (!enoentRe.test(error.toString())) {
        logger.error(error);
      }
    }
    if (components == null) {
...
const loadNpm = (config, cb) => {
  ...
  items = Object.keys(json.dependencies || {}).filter(dep => {
    ...
    if ((ref = json.overrides) != null ? ref[dep] : undefined) {
...
const addPackageManagers = (config, callback) => {
...
exports.loadConfig = (configPath, options, callback) => {
  ...
  return addPackageManagers(config, () => {
    deepFreeze(config);
