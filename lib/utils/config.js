...
const normalizeLoggyConfig = config => {
  if (config === false) return false;

  const brunchDefaults = {
    app: 'Brunch',
  ...
  if (config === true) {
    ...
    return brunchDefaults;
  }

  if (Array.isArray(config)) {
    logger.warn('`config.notifications` array is deprecated. Use `config.notifications.levels` instead.');
    return Object.assign({}, brunchDefaults, {levels: config});
  }

  return Object.assign({}, brunchDefaults, config);
};

const setConfigDefaults = (config, configPath) => {
  Object.assign(logger.notifications, normalizeLoggyConfig(config.notifications));
