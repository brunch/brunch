...
const loadPackages = (rootPath) => {
  ...
  const plugins = loadDeps(keys(json.dependencies));
  const devPlugins = loadDeps(keys(json.devDependencies), true);
  const optPlugins = loadDeps(keys(json.optionalDependencies), true);
...
const initialize = (options, configParams, onCompile) => {
  ...
  return application.loadConfig(options.config, configParams).then(config => {
    ...
    const offP = config.plugins.off || [];
    const onlyP = config.plugins.only || [];
    const packages = (loadPackages('.')).filter(arg => {
      const brunchPluginName = arg.brunchPluginName;
      if (offP.length && offP.indexOf(brunchPluginName) >= 0) {
        return false;
      } else if (onlyP.length && onlyP.indexOf(brunchPluginName) === -1) {
        return false;
      } else {
        return true;
      }
    ...
    const launchWatcher = () => {
      ...
      return initWatcher(config).then(watcher => {
        ...
        const compile = getCompileFn(config, joinConfig, fileList,
          optimizers, watcher, preCompilers, callCompileCallbacks);
  ...
  constructor(persistent, options, onCompile) {
    ...
    initialize(options, configParams, onCompile)
      .then(result => {
        ...
        bindWatcherEvents(
          config, fileList, compilers, linters,
          watcher, reload, this._startCompilation
        );
        ...
        fileList.on('bundled', () => this._endBundle());
