...
const generateCompilationLog = (startTime, copiedAssets, generatedFiles, disposedFiles) => {
  ...
  const diff = formatTime(Date.now() - startTime);

  return `${main || 'compiled'} in ${diff}`;
};

const formatTime = ms => {
  if (ms < oneSecond) return `${ms} ms`;

  const sec = (ms / oneSecond).toFixed(1);
  return `${+sec} sec`;
};
...
const writeFile = (path, data) => {
  ...
  return mkdirp(dir, {mode}).then(() => {
    return fsWriteFile(path, data, {mode});
  });
};
