  ...
  return compiler.compileStatic(file).then(compiled => {
    ...
    return deps.then(dependencies => {
      ...
      return Object.assign({}, compiled, {
        dependencies,
        path: path.replace(extRe, compiler.staticTargetExtension),
      });
    });
...
const processAsset = parallel('PROCESS_ASSET', file => {
  ...
  const match = file => compiler => compiler.staticPattern.test(file.path);
  const nextCompiler = file => {
    const compiler = pull(compilers, match(file));
    if (!compiler) return file;

    return compileStatic(file, compiler).then(nextCompiler);
  };

  return compilers.some(match(file)) ?
    file.hydrate().then(nextCompiler) :
    file;
});
