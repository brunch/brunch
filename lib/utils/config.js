...
const setConfigDefaults = (config, configPath) => {
  logger.notifications = (levels => {
    if (levels === false) return levels;
    if (levels === true) {
      logger.warn('`config.notifications: true` is deprecated. Notifications are on by default.');

      // Use Loggy's defaults
    ...
    if (Array.isArray(levels)) {
      ...
      // Mix with Brunch's defaults
      return {
        app: 'Brunch',
        icon: sysPath.resolve('..', '..', 'logo.png'),
        levels,
      };
    }
