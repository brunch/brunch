...
const promisifyPlugin = (arity, fn) => fn.length === arity ? fn : promisify(fn);
...
const callPlugin = (plugin, fn, arity, args, msg) => {
  const promise = promisifyPlugin(arity, fn).apply(plugin, args);
  if (!msg) msg = 'process';

  const warningLogInterval = 15000;

  return safePromise(promise, warningLogInterval,
