...
const changedSince = startTime => generated => {
  return generated.allSourceFiles.some(f => f.compilationTime >= startTime);
};
...
const writeStatic = (fileList, startTime) => {
  const assetErrors = fileList.assetErrors;
  if (assetErrors.length) throw assetErrors.join(' ; ');

  const changed = fileList.copiedAfter(startTime);
  const written = changed.map(file => file.write());

  debug(`Writing ${changed.length} assets`);
...
const write = (fileList, config, joinConfig, optimizers, startTime) => {
  ...
  const errors = files
    .map(generated => {
      return generated.sourceFiles.filter(f => f.error).map(formatError);
    })
  ...
  const statics = writeStatic(fileList, startTime);
  ...
  return Promise.all(generated.concat(statics)).then(() => changed);
};
