...
function launchServerFileModule(server, port, publicPath, serverConfig) {
  ...
  if (typeof launcher !== 'function') {
    throw new TypeError('Brunch server file needs to export server function');
  }
...
class BrunchServer {
  ...
  defaultStaticServer() {
    ...
    const mergedOptions = Object.assign({}, staticOptions, {public: publicPath})
    ...
    return new Promise((resolve, reject) => {
      ...
      server.listen(port, hostname, () => {
        ...
        if (!ifshow) logger.info(`application started on http://${hostname}:${port}/`);
