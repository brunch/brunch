...
const load = (config, json, deps) => new Promise((resolve, reject) => {
  ...
  const detect = i => {
    const deps = detective(i.data).filter(x => shimKeys.indexOf(x) === -1 && Object.keys(globalVrowserMappings(i.file)).indexOf(x) === -1);
    return { file: i.file, deps: detective(i.data).filter(x => shimKeys.indexOf(x) === -1) };
  };
