...
const {pull} = require('./helpers');
const {respondTo, npmCompilers} = require('./plugins');
...
const lint = file => {
  ...
  return Promise.all(linters).then(results => {
    results.forEach(res => {
      if (res === Object(res)) {
...
exports.processAsset = job('PROCESS_ASSET', file => {
...
exports.processFile = job('PROCESS_FILE', file => {
  const {isNpm, path} = file;
  const compilers = isNpm ? respondTo('compile') : npmCompilers;

  const nextCompiler = file => {
    const compiler = pull(compilers, compiler => {
      return compiler.pattern.test(path);
    });
