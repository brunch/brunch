...
const buildModMap = npmConfig => {
  ...
  return glob(`{${topPaths}}/**/*`).then(mods => {
    ...
    const shimModMap = shims.emptyShims.filter(noPackage(npmConfig.rootPackage)).reduce((acc, shim) => {
      acc[shim] = shims.makeSpecialShimFname(shim);
      return acc;
    }, {});

    const shimFileMap = Object.keys(shims.fileShims).filter(noPackage(npmConfig.rootPackage)).reduce((acc, shim) => {
...
const friendlyRequireError = (rootPackage, mod, opts, err) => {
...
const resolve = npmConfig => (mod, opts, cb) => {
  ...
  const modMap = npmConfig.modMap;
  const aliases = npmConfig.npm.aliases;

  const packageFilter = pkg => helpers.applyPackageOverrides(pkg, rootPackage);
  ...
  browserResolve(mod, opts, (err, res) => {
    if (err) {
      return cb(friendlyRequireError(rootPackage, mod, opts, err));
    }
