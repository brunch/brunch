...
const threaded = require('../workers').threaded;
...
const runOptimizer = optimizer => params => {
  ...
  debug(`Optimizing ${path} @ ${optimizer}`);

  const data = params;
  params.brunchPluginName = optimizer;

  return threaded('OPTIMIZE_FILE', data).then(optimized => {
...
const generate = (path, targets, config, optimizers) => {
  ...
  const foptim = optimizers.filter(optimizer => optimizer.type === type).map(opt => opt.brunchPluginName);
