...
const prepWrapper = (compiled, wrapped) => {
...
class SourceFile {
  constructor(path, npmStatic, vendorConvention, moduleWrapper, fileList) {
    ...
    this.type = deppack.isNpmJSON(path) ? 'javascript' : null;
    this.targets = {};
  ...
  _updateCache(file) {
    ...
    return this._updateMap(data, wrapped, file.sourceMap).then(node => {
      this.targets = {
        [this.type]: {data, node},
      };
      ...
      this.targets.javascript = {
        data: exports,
        node: wrappedNode(this.path, exports, this._wrap(exports)),
      };
    });
  }
  ...
  _updateMap(compiled, wrapped, sourceMap) {
    const path = this.path;
