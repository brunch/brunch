...
const deepExtend = (object, properties, allObjects) => {
  if (allObjects == null) allObjects = {};
  const nestedObjs = Object.keys(allObjects).map(file => allObjects[file]);
  Object.keys(properties).forEach(key => {
    const value = properties[key];
    if (toString.call(value) === objType && nestedObjs.indexOf(object) === -1) {
      if (object[key] == null) object[key] = {};
      deepExtend(object[key], value, allObjects);
    } else {
      object[key] = value;
    }
  });
