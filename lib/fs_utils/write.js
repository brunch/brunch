...
const writeStatic = (fileList, config, startTime) => {
  ...
  const errors = staticFiles.filter(f => f.error != null).map(formatError);
...
const writeSWPreCache = config => {
  const path = sysPath.join(config.paths.public, config.offlineFirst.swFileName);

  return SWPreCache.generate(config.offlineFirst.options).then(fileContent => {
    return generate.writeFile(path, fileContent);
  });
};
...
const write = (fileList, config, joinConfig, optimizers, startTime) => {
  ...
  return Promise.all(changed.map(file => {
    ...
    if (config.offlineFirst.enabled) return writeSWPreCache(config);
  }).then(() => {
