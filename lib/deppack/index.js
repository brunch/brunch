...
class Deppack {
  ...
  processFiles(root, files, processor) {
    const npmConfig = this.npmConfig;
    const requiredAliases = modules.requiredAliases(npmConfig.rootMainCache, npmConfig.rootPackage, npmConfig.usedShims);
  ...
  wrapInModule(filePath) {
    ...
    const npmConfig = this.npmConfig;
    return readFile(filePath).then(src => modules.wrap(npmConfig.rootPackage, npmConfig.rootMainCache, filePath, src));
  }
