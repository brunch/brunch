...
class Deppack {
  constructor(config, json) {
    ...
    this.fileReflection = fileReflection(this.npmConfig);
    this.resolve = xBrowserResolve.resolve(this.npmConfig);
  }

  init() {
    return xBrowserResolve.buildModMap(this.npmConfig)
      .then(() => loadGlobalsAndStyles(this.npmConfig, this.resolve));
  }

  exploreDeps(sourceFile) {
    return explore(this.npmConfig, this.fileReflection, this.resolve)(sourceFile);
  }
