...
async function clone(skeleton) {
  ...
  if (await exists(repoDir)) {
    logger.info(`Pulling recent changes from git repo "${url}" to "${repoDir}"...`);

    try {
      await exec('git pull origin master', {cwd: repoDir});
      logger.info(`Pulled master into "${repoDir}"`);
      return repoDir;
    } catch (error) {
  ...
  logger.info(`Cloning git repo "${url}" to "${repoDir}"...`);
  try {
    await exec(`git clone ${url} "${repoDir}"`);
    logger.info(`Cloned "${url}" into "${repoDir}"`);
    return repoDir;
  } catch (error) {
...
async function initSkeleton(alias, rootPath) {
  ...
  logger.info(`Copying local skeleton to "${rootPath}"...`);
