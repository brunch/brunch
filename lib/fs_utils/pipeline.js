...
const deppack = promisify(require('../deppack').loadFile);
...
const lint = (data, path, linters) => {
  const reduceLinter = (previousPromise, linter) => {
    ...
    return previousPromise.then(() => {
      return promisify(linter.lint).call(linter, data, path);
    }, err => {
...
const getDependencies = (data, path, compilerDeps, compiler) => {
  if (compiler.getDependencies) {
    ...
    debug(`getDependencies ${path} @ ${name}`);
...
const isNpm = path => {
  ...
  return path.indexOf('node_modules') >= 0 && !brre.test(path) && path.indexOf('.css') === -1;
