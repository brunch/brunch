...
const updateCache = (path, cache, error, result, wrap, jsWrap) => {
  ...
  if (result == null) {
    cache.error = null;
    cache.targets = {
      [cache.type]: {data: null, node: null},
    };
...
class SourceFile {
  constructor(path, compilers, linters, wrapper, isHelper, isVendor, fileList, depCompilers) {
    ...
    this.compile = () => pipeline(this.path, linters, compilers, this.fileList, depCompilers)
      .then(data => {
        if ('path' in data) {
          this.path = data.path;
        }
        return updateCache(this.path, this, null, data, wrap, jsWrap).then(() => null);
      }, error => {
        return updateCache(this.path, this, error, null, wrap, jsWrap).then(() => {
          throw error;
        });
      });
    ...
    Object.preventExtensions(this); // Disallow adding new properties.
  }
