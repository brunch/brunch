...
const runOptimizer = (optimizer, params) => {
  ...
  const scheduleOptimize = () => {
    return mediator.wrkr.schedule('optimize', {optimizer: optimizer.constructor.brunchPluginName, params: {data, path, map}});
  };

  return (mediator.wrkr ? scheduleOptimize : _optimize)()
    .then(optimized => {
